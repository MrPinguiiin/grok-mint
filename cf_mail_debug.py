...
def generate_username(length: int = 10) -> str:
    """Menghasilkan nama email acak yang diperlukan oleh admin API cloudflare_temp_email."""
    chars = string.ascii_lowercase + string.digits
    return "".join(secrets.choice(chars) for _ in range(length))


def normalize_path(path: str, default_path: str) -> str:
    """Menormalisasi path API untuk menghindari hilangnya garis miring di awal."""
...
def build_auth_headers(auth_mode: str, api_key: str, content_type: bool = False) -> Dict[str, str]:
    """Membuat header autentikasi antarmuka email sementara Cloudflare berdasarkan parameter debug."""
...
def create_address(
    ...
    """Membuat alamat email sementara Cloudflare, mendukung API anonim dan API admin."""
